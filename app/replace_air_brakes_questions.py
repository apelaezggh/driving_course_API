...
def replace_air_brakes_questions():
    db = SessionLocal()
    try:
        ...
        existing_questions = db.query(Question).filter(Question.topic_id == topic.id).all()
        for question in existing_questions:
            db.delete(question)
        print(f"Deleted {len(existing_questions)} existing questions")
        ...
        questions_data = [
            ...
            {
                "question_en": "What pressure is required for the low pressure warning signal to deactivate?",
        ...
        db.commit()
        print(f"\nSuccessfully replaced and added {added_count} questions to 'Frenos de Aire'")
        print(f"Total questions in topic: {db.query(Question).filter(Question.topic_id == topic.id).count()}")
        
    except Exception as e:
        print(f"Error replacing questions: {e}")
        db.rollback()
    finally:
        db.close()

if __name__ == "__main__":
    replace_air_brakes_questions() 